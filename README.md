...
- **Stroke weight**, with independent **horizontal** and **vertical** control: thicken the vertical stems without touching the horizontal bars (or the reverse), so you can dial a font's stroke contrast. Weight is shown **live** everywhere — preview lines, the pair test and the zoom view — by rebuilding the glyph as a raster with the same per-axis morphology as export (grow stems for horizontal, bars for vertical), updated as you drag. The exported geometry uses the same morphology re-traced back to outlines. One-click weight equalization too — even out stem weight or bar weight across capitals, lowercase or all glyphs.
